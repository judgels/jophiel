...
import org.iatoki.judgels.jophiel.controllers.api.pub.v1.user.routes;
...
final class UserServiceUtils {
    ...
    static URL getAvatarImageUrl(String imageName) {
        try {
            return new URL(routes.PublicUserAPIControllerV1.renderAvatarImage(imageName).absoluteURL(Http.Context.current().request(), Http.Context.current().request().secure()));
